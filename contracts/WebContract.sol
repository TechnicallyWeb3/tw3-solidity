...
contract WebContract {
    ...
    mapping(string path => string content) private htmlContent;
    mapping(string path => string content) private jsContent;
    mapping(string path => string content) private cssContent;
    mapping(string path => string[] contents) private directory;
    ...
    function addPathComponents(string calldata path) internal {
        ...
        // string memory directoryCopy = directory;
    ...
    function removePathComponents(string calldata path) internal {
        ...
        for (uint256 i = pathComponents.last(); i > 0; i--) {
            ...
            currentDirectory = currentDirectory.slice(0, int256(componentLength + offset));
            // Remove the component from the current directory
            while (directory[currentDirectory].includes(pathComponents[i])) {
                directory[currentDirectory] = directory[currentDirectory].remove(directory[currentDirectory].indexOf(pathComponents[i]));
            }
        }
    }
}